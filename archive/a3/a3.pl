...
% the program
prog_ast(program(X)) --> reptprog0_ast(X).
...
structdecl_ast(struct(id(ID),Opt,ReptStruct)) --> {length(ID,1)},
						  ["struct"],id(ID),
						  structinherits_ast(Opt),
					     ["{"], reptstructdecl4_ast(ReptStruct),
					     ["}"], [";"].

reptstructdecl4_ast([struct_declaration(visibility(Vis),Mem)|N]) --> visibility_ast(Vis),
				       memberdecl_ast(Mem),
...
%@    X = program([func_def(func_head(id([Y]),[],type(["void"])),func_body([])),struct(id(["POLYNOMIAL ..."]),[],[struct_declaration(visibility("public"),member(func_decl(func_head(id(["eval ..."]),func_param(id(["x"]),type("float ..."),[],[]),type("float")))))])])
%@ ;  ...
...
%?- phrase(statement_ast(X),["id","(","id",")",".","id","(",")",";"]),!.
%@    X = assign(id(["id"]),parens([array_parameter(expr(arith_expr(term(factor(id(["id"]),[],[]),[]),[]),[]))],nested_id(id(["id"]),parens([array_parameter(expr(arith_expr(term(factor(id([]),[],[]),[]),[]),[]))],[])))).
